    ...
    for(yy in y){
      ...
      if(!identical(xx,yy)) {
        if(is.null(l)) l=rbind(l,c(xx,yy)) else 
          if(!any(apply(l,1,function(x)identical(x,c(xx,yy))))) if(!any(apply(l,1,function(x)identical(rev(x),c(xx,yy)))))  l=rbind(l,c(xx,yy))
      }
    }
  }
  library(plyr)
...
#   labss=lapply(names(df), function(x) {l=attr(df[var],"label");if(is.null(l)) x else x})
  labss=lapply(as.character(var), function(x) {l=attr(df[,x],"label");if(is.null(l)) x else x})
...
otrans=function(xx,yy=NULL,zz=NULL,xfilter=NULL,yfilter=NULL,usemultichoicelabs=F,table=F){
  library(dplyr)
#               browser()
  if(!xexists(msg))msg=""
#   browser()
...
if(table){
  ...
  if(is.table(xx))stop("you said xx is a table but I meant a dataframe in the form of a table.")
  xx$id=row.names(xx)
  m=melt(xx,id="id")
  xx=m[,1]
  yy=m[,2]
...
#           })) 
      ...
      #       browser()
      # ggheat:
      if(length(dim(xx))==2 & length(dim(yy))==2 & is.null(yfilter)) {
        library(stringr)
        xdat=xx
        ydat=yy
#                 browser()
...
#         browser()
    ...
    #     if(!all(lapply(xx,function(i)0==length(dim(xx))))) stop ("You have given a list but they are not all vectors")
    ...
    #     tmp=lapply(xx,function(i)data.frame(findlabs(melt(i,id=NULL),i),labb(i)))#so if the first one is a factor the next ones have to accommodate to it 
#         browser()
    
    tmp=lapply(xx,function(i){
      df=data.frame(xx=melt(i,id=NULL),yy=paste0(attr(i,"datafilename"),""))
#       df[,1]=str_trim(findlabs(df[,1],i))
      df
...
#         o=summarise(group_by(o,variable,labb.i.),mean(value))
        o=summarise(group_by(o,variable,yy),mean(xx.value))
        zz=o[,3]
      }
    }
        yy=factor(o[,2])
  ...
  ### so we pre-processed blocks and lists into xx and yy; now we can continue with the xx and yy processing
p=otest(xx,yy)
### it is just a single xx var for histograms:
  if (is.null(yy)) oo=data.frame(xx)  
  else {
  ### discrete_discrete
